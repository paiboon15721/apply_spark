...
def get_prediction(tweet_text):
    try:
        ...
        results = pipelineFit.transform(wordsDataFrame).select(
            'tweet', 'prediction').collect()
        requests.post('http://localhost:3000/predictions', json=results)
    except:
        requests.post('http://localhost:3000/predictions',
...
if __name__ == "__main__":
    ...
    my_data = spark.read.csv(
        './twitter_sentiments.csv', schema=my_schema, header=True)
    my_data.show(2)
